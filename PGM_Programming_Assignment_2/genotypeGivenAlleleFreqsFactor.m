...
for i = 1: size(assignments, 1)
    prob = 1;
    allele1 = genotypesToAlleles(assignments(i), 1);
    allele2 = genotypesToAlleles(assignments(i), 2);
    if(allele1 ~= allele2)
        prob = prob * 2 * alleleFreqs(allele1) * alleleFreqs(allele2);
    else
        prob = prob * alleleFreqs(allele1) * alleleFreqs(allele2);
    end
