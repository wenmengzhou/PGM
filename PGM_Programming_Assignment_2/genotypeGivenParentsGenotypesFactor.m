...
genotypeNum = numAlleles * (numAlleles-1) / 2  + numAlleles;
...
for i = 1: size(assignments, 1)
    allelesc  = genotypesToAlleles(assignments(i, 1), :);
    allelesp1 = genotypesToAlleles(assignments(i, 2), :);
    allelesp2 = genotypesToAlleles(assignments(i, 3), :);
    
    count = 0;
    for m = 1:2
        for n = 1:2
            if sum(allelesc == [allelesp1(m), allelesp2(n)]) == 2  || sum(allelesc == [allelesp2(n), allelesp1(m)]) == 2
                count = count + 1;
            end
        end
    end
    genotypeFactor = SetValueOfAssignment(genotypeFactor, assignments(i,:), count/4);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%